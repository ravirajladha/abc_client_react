...
const SettingsModal = () => {
    ...
    const closeSettingsModal = () => {
        setSettings({ ...settings, settingsModalIsOpen: false, dropdownIsOpen: false });
    }
        ...
        <div className='text-white z-40'>
            <div aria-modal='true' role='dialog' className='fixed inset-0 overflow-y-auto z-modal'>
                <div className='flex min-h-screen items-center justify-center px-4'>
                    ...
                    {/* <div className='fixed inset-0 opacity-100' onClick={closeSettingsModal}></div> */}
                    <div className='fixed inset-0 opacity-100' onClick={closeSettingsModal}></div>
            ...
            <div className='relative'>
                <button onClick={handleClickDropdown} className='flex cursor-pointer items-center rounded px-3 py-1.5 text-left focus:outline-none whitespace-nowrap bg bg-dark-fill-3 hover:bg-dark-fill-2 active:bg-dark-fill-3 w-full justify-between'>
                    {fontSize}
                    <BsChevronDown />
                </button>
                {settings.dropdownIsOpen && (
                    <ul className='absolute mt-1 max-h-56 overflow-auto rounded-lg p-2 z-50 focus:outline-none shadow-lg w-full bg-dark-layer-1' style={{
    ...
    <SettingsListItem 
        ...
        handleFontSizeChange={handleFontSizeChange} // <-- passing down the function
    />
))}
                    </ul>
                )}
